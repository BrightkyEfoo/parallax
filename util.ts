...
// Chiffrement
export function encryptMessage(message: Buffer, iv: CryptoJS.lib.WordArray) {
...
export async function formDataToBuffer(formData: FormData) {
  ...
  for (const [key, value] of formData.entries()) {
    ...
    if (value instanceof Blob) {
      ...
      chunks.push(Buffer.from(buffer).toString("hex"));
    } else {
      chunks.push(
...
export function bufferToFormData(buffer: Buffer, boundary: string) {
    ...
    if (part.includes("Content-Disposition")) {
      ...
      if (nameMatch) {
        ...
        if (filenameMatch) {
          ...
          const blob = new Blob([Buffer.from(content, "hex")], { type });
